...
export function DispensePrescriptionItemForm({
  ...
  // Get selected batch info for confirmation
  const selectedBatchId = formData?.get("batch_id") as string;
  const selectedBatch = batches.find(b => b.id === selectedBatchId);
  const quantity = formData?.get("quantity_dispensed") as string;

  const facts = [
    { label: "Medication", value: item.medication_name, highlight: true },
    ...(selectedBatch ? [{ label: "Batch", value: selectedBatch.batch_number }] : []),
    ...(quantity ? [{ label: "Quantity", value: quantity }] : []),
  ];

  return (
    <>
      <form action={handleSubmit} className="rounded-xl border bg-background p-4">
      ...
      <ConfirmationModal
        ...
        description="This will reduce stock from the selected batch and mark the item as dispensed."
        onConfirm={handleConfirm}
        confirmLabel="Confirm Dispensing"
        pending={pending}
      >
        <ConfirmationPanel
          title="Review Before Dispensing"
          severity="warning"
          facts={facts}
        />
      </ConfirmationModal>
